...
class ItemActivity : AppCompatActivity() {
    ...
    private lateinit var offlineModeItem: MenuItem
        ...
        filterSwitch.setOnCheckedChangeListener { _, isChecked ->
            CoroutineScope(Dispatchers.IO).launch {
                realm.subscriptions.update {
                    ...
                    val queryMineCompleted = realm.query<Item>(
                        "owner_id == $0 && isComplete == $1",
    ...
    override fun onCreateOptionsMenu(menu: Menu): Boolean {
        ...
        offlineModeItem = menu.findItem(R.id.action_airplane_mode)

        if (this::realm.isInitialized) {
            if (connectivityEnabled) {
                offlineModeItem.icon = syncEnabledIcon
                realm.syncSession.resume()
            } else {
                offlineModeItem.icon = syncDisabledIcon
