...
export function ItemListView() {
  ...
  useEffect(() => {
    ...
    console.log('setting logger: ', app);
  ...
  useEffect(() => {
    const ownItems = realm
      ...
      .filtered(
        `owner_id == "${user?.id}"${
          showCompletedItems ? ' AND isComplete == true' : ''
        }`,
      );
    ...
    <SafeAreaProvider>
      <View style={styles.viewWrapper}>
        <View style={{flexDirection: 'row', alignItems: 'center'}}>
          <Text style={{flex: 1, padding: 10, fontSize: 16}}>
        ...
        {/*<Button
          ...
          buttonStyle={styles.showCompletedButton}
          onPress={() => setShowCompletedItems(!showCompletedItems)}
          icon={
            <Icon
              type="material"
              name={showCompletedItems ? 'view-headline' : 'playlist-add-check'}
